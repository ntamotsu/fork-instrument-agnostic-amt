...
def _replace_fixed_loudness_controls_in_mido(midi: mido.MidiFile) -> None:
    ...
    _strip_loudness_controls_in_mido(midi)
    ...
    for track, channels in zip(midi.tracks, note_channels_by_track):
        ...
        track[insert_at:insert_at] = fixed_controls


def _strip_loudness_controls_in_mido(midi: mido.MidiFile) -> None:
    '''CC7/CC11をdelta timeを保ったまま全trackから除去する。'''
...
def _apply_loudness_controls_in_mido(
    midi: mido.MidiFile,
    policy: str,
) -> None:
    if policy == 'preserve':
    ...
    if policy == 'velocity_only':
        _replace_fixed_loudness_controls_in_mido(midi)
        return
    raise ValueError(
...
def _write_velocity_template_midi(
    ...
    *,
    ...
    _apply_loudness_controls_in_mido(template_midi, loudness_controls)
