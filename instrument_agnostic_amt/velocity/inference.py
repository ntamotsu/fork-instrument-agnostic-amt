...
def predict_velocity_values(
    *,
    ...
    if len(stem_waveforms) != len(stem_class_ids) or not stem_waveforms:
        raise ValueError("stem_waveforms and stem_class_ids must be non-empty and aligned")
    if window_seconds <= 0:
    ...
    starts = np.asarray([note.start_seconds for note in notes], dtype=np.float32)
    ends = np.asarray([note.end_seconds for note in notes], dtype=np.float32)
    ...
    if reject_out_of_range and (
        np.any(starts < 0.0) or np.any(starts >= total_duration_seconds)
    ):
    ...
    window_starts_seconds = (
        ...
        else list(np.arange(0.0, total_duration_seconds, window_seconds))
    )
    ...
    with torch.inference_mode():
        for window_start in tqdm(
            ...
            if select_note_stems_only:
                ...
                window_stem_class_tensor = stem_class_tensor
                window_valid_audio_frames = valid_audio_frames
            if pad_final_window and int(sub_audio.shape[-1]) < window_samples:
            ...
            def tensor(values: np.ndarray, *, dtype: torch.dtype) -> torch.Tensor:
                return torch.from_numpy(values).unsqueeze(0).to(device=device, dtype=dtype)

            forward_kwargs: dict[str, Any] = {
                "note_start_seconds": tensor(
                    (starts[indices_in_window] - window_start).astype(np.float32),
                    dtype=torch.float32,
                ),
                "note_end_seconds": tensor(
                    (ends[indices_in_window] - window_start).astype(np.float32),
                    dtype=torch.float32,
                ),
